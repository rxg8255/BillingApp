    ...
    // }
      ...
      const rowSelection = {
        onChange: async (selectedRowKeys, selectedRows) => {
          ...
          await selectedRows.map((item) => item['count'] = <InputNumber name='name' min={1} max={item.available} defaultValue={1} onChange= {(e) => { this.setState({changeData: e * item.mrp_price});this.setState({changeKey:item.key})}} />);
